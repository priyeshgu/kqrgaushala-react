...
const DonationContent = ({ handleLogout }) => {
  ...
  useEffect(() => {
    ...
    fetch('http://192.168.1.96:3001/donationCategories?type=products')
  ...
  const handleSaveClick = async (updatedProduct) => {
    try {
      const response = await fetch('http://192.168.1.96:3001/updateProduct', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteClick = (productId, name_in_english, name_in_hindi, type, cost) => {
    ...
    if (confirmDelete) {
      ...
      try {
        fetch('http://192.168.1.96:3001/deleteProduct', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleAddProduct = async () => {
    try {
      const response = await fetch('http://192.168.1.96:3001/addProduct', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
